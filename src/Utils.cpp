...
{
    if (path1.empty())
        return path2[0] == '/' ? path2.substr(1) : path2;
    if (path2.empty())
        return path1[0] == '/' ? path1.substr(1) : path1;
    if ((*(path1.rbegin()) == '/' && *(path2.begin()) != '/') || (*(path1.rbegin()) != '/' && *(path2.begin()) == '/'))
        return (path1[0] == '/' ? path1.substr(1) : path1) + path2;
    return (path1[0] == '/' ? path1.substr(1) : path1) + "/" + path2;
}
...
bool isCGIMatch(std::string const &path1, std::string const &path2)
{
    ...
    if (pos == std::string::npos)
        return false;
    std::string ext1 = path1.substr(pos);
    pos = path2.find_last_of('.');
    if (pos == std::string::npos)
        return false;
    std::string ext2 = path2.substr(pos);
    return (ext1 == ext2 && hasCGIExtension(path1));
}